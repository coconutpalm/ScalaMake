...
case class Rule(_target : String) {
  ...
  private var action0 : () => Any = {() =>  }
  private var action1 : (String) => Any = { file => }
  ...
  def buildWith(action : => Any) = {
    this.action0 = { () => action }
  }
  def buildWith(action : (String) => Any) = {
...
//    println(s"$rule : ${rule.dependencies}\n $ruleSet\n")
  ...
  private def runRule(rule : Rule, ruleSet : HashMap[String,Rule]) : Any = {
  ...
  def runRule(ruleTarget : String, ruleSet : HashMap[String,Rule]) : Any = {
